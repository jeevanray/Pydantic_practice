...
@retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=2, max=10),
       retry=retry_if_exception_type(Exception))
def connect_to_oracle(oracle_conf: Dict[str, Any] | None) -> oracledb.Connection:
    if oracle_conf is None:
        raise ValueError("Oracle configuration is required")
...
def update_audit_record_strict(config_audit: Dict[str, Any], audit_data: Dict[str, Any],
    ...
    required_fields = [
        "source_table", "business_loaddt", "load_type", "status",
        "task_startts", "task_endts", "task_exec_secs"
    ]
    
    for field in required_fields:
        if field not in audit_data:
            logger.warning(f"Missing required field {field} in audit data")
            if field in ["task_startts", "task_endts"]:
                audit_data[field] = datetime.now(IST)
            elif field == "task_exec_secs":
                audit_data[field] = 0
            elif field == "load_type":
                audit_data[field] = "delta"
            elif field == "status":
                audit_data[field] = "NOT_STARTED"
            else:
                raise KeyError(f"Required field {field} missing in audit data")

    # Ensure schema is present
    if "schema" not in config_audit:
        raise KeyError("schema is required in config_audit")
    ...
    for field, default_value in default_values.items():
        if field not in audit_data:
            audit_data[field] = default_value

    audit_data_copy = _ensure_time_fields(audit_data)
    logger.debug(f"Prepared audit data: {audit_data_copy}")
...
def get_historic_load_status(config_audit: Dict[str, Any], source_table: str,
                           current_business_loaddt: str, delta_column: Optional[str],
                           delta_column_type: Optional[str], delta_column_format: Optional[str],
                           oracle_config: Optional[Dict[str, Any]]) -> List[Dict[str, Any]]:
...
def update_audit_table_schema(config_audit: Dict[str, Any], conn, cur) -> None:
    """Update the audit table schema with any missing columns."""
    try:
        ...
        cur.execute(check_columns_query, 
                   (config_audit["audit_table"].upper(), 
                    config_audit["schema"].upper()))
        ...
        new_columns = {
            ...
            'API_FAILEDPATH': 'CLOB',
            'APICALL_TIME': 'NUMBER',
            'CDP_DB_COUNT_VALIDATION': 'CHAR(1)',
            ...
            'LOG_PATH': 'VARCHAR2(500)',
            'MINIO_FILEPATH': 'VARCHAR2(500)'
        }
        ...
        for column_name, column_type in new_columns.items():
            if column_name not in existing_columns:
                alter_query = f"""
                ALTER TABLE {config_audit["schema"]}.{config_audit["audit_table"]} 
                ADD {column_name} {column_type}
                """
                try:
                    cur.execute(alter_query)
                    logger.info(f"Added column {column_name} to audit table")
                except Exception as e:
                    logger.warning(f"Failed to add column {column_name}: {e}")
        
        conn.commit()
        logger.info("Completed audit table schema updates")
        
    except Exception as e:
        conn.rollback()
        logger.error(f"Failed to update audit table schema: {e}")
        raise
...
def create_audit_table_if_not_exists(config_audit: Dict[str, Any]) -> None:
    ...
    cur = None
    try:
        conn = get_oracle_connection(config_audit["target"])
        conn.autocommit = False
        cur = conn.cursor()
        ...
        if not exists:
            ...
            CREATE TABLE {config_audit["schema"]}.{config_audit["audit_table"]} (
                ...
                CREATED_AT_TS TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                UPDATED_AT_TS TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                CONSTRAINT PK_{config_audit["audit_table"]} PRIMARY KEY (ID)
            )
            """
            cur.execute(create_table_sql)
            conn.commit()
            logger.info(f"Created audit table {config_audit['schema']}.{config_audit['audit_table']}")
        else:
            logger.info(f"Audit table {config_audit['schema']}.{config_audit['audit_table']} already exists")
            
        # Update schema with any missing columns
        update_audit_table_schema(config_audit, conn, cur)
            
    except Exception as e:
        if conn:
            conn.rollback()
        logger.error(f"Failed to create/update audit table: {e}")
        raise
    finally:
        close_connection(cur, conn)
...
def process_from_config(config: Dict[str, Any], conn_config: Dict[str, Any],
    ...
    audit_config = {
        ...
        "schema": "appuser",  # Using appuser schema as per your Oracle setup
    ...
    try:
        ...
        logger.error(f"Failed to create/verify audit table: {e}")
        raise
...
if __name__ == "__main__":
    ...
    try:
        ...
        required_config = {
            "target": ["host", "port", "user", "password", "service_name"],
            "minio": ["endpoint", "access_key", "secret_key"]
        }
        
        for section, required_fields in required_config.items():
            if section not in conn_config:
                raise ValueError(f"Missing required configuration section: {section}")
            for field in required_fields:
                if not conn_config[section].get(field):
