...
def initialize_restart_audit_log(config_audit: Dict[str, Any], audit_log: Dict[str, Any], aud_dt: str, 
    ...
    if delta_column_value:
        where_clause += " AND DELTA_COLUMN_VALUE = :delta_val"
...
def update_audit_record(config_audit: Dict[str, Any], audit_data: Dict[str, Any]) -> None:
    """Upsert audit row by (source_table, business_loaddt, delta_column_value) using bind variables."""
    ...
    merge_sql = f"""
        ...
        ON (target.source_table = src.source_table 
            ...
            AND NVL(target.delta_column_value, 'NULL') = NVL(src.delta_column_value, 'NULL')
            AND NVL(target.load_type, 'NULL') = NVL(src.load_type, 'NULL'))
...
def get_load_status_and_dates(config_audit: Dict[str, Any], source_table: str, 
                             ...
                             delta_column: Optional[str] = None) -> List[Dict[str, Any]]:
    """
    ...
    """
    if load_type == 'historic' and delta_column:
        return get_historic_load_status(config_audit, source_table, current_business_loaddt, delta_column)
    else:
...
def get_historic_load_status(config_audit: Dict[str, Any], source_table: str, 
                           current_business_loaddt: str, delta_column: str) -> List[Dict[str, Any]]:
    """Get historic load status - process one delta value at a time in sorted order."""
    
    # First, get the next unprocessed delta value
    next_delta_query = f"""
        ...
        WHERE sd.delta_value NOT IN (SELECT delta_column_value FROM processed_deltas)
    """
    ...
    try:
        ...
        cur.execute(next_delta_query, {"src": source_table})
        result = cur.fetchone()
        ...
        # Check if this delta value is already being processed
        status_query = f"""
            SELECT
                business_loaddt,
                NVL(status, 'NOT_STARTED') AS status,
        ...
        if row:
            ...
            return [{
                ...
                "restart_point": int(restart_point or 0),
                "total_records": int(total_records or 0),
                "delta_column_value": str(next_delta),
                "load_type": 'historic'
            }]
        else:
        ...
        if exists == 0:
            ...
            create_sql = f"""
                CREATE TABLE {config_audit['schema']}.{config_audit['audit_table']} (
                    ...
                    CONSTRAINT uq_source_bizdate_delta UNIQUE (source_table, business_loaddt, delta_column_value, load_type)
                )
            """
...
def generate_object_path(base_path: str, business_loaddt: str, load_type: str, 
    ...
    if load_type == 'historic':
        # For historic: base_path/history/sub_folder_value
        if delta_column_value and sub_folder:
            return f"{base_path}/history/{delta_column_value}"
        else:
            return f"{base_path}/history/{date_folder}"
    else:
...
def process_oracle_to_minio_with_dependencies(
    ...
    *,
    ...
    dates = get_load_status_and_dates(config_audit, table_name_uc, current_business_loaddt, load_type, delta_column)
    if not dates:
        ...
        "objects": [
            {
                ...
                "delta_column": "AUD_DT", 
                "sub_folder": "AUD_DT",
                "output_path": "test/historic_data",
                "ORDER_BY": "AUD_DT",
                "chunksize": 1000
            }
        ]
