...
# Logging
logger = logging.getLogger("Mongo_MinIO")
...
def upload_arrow_table_parquet(minio_client: MinioHandler, table: pa.Table,
    ...
    """Upload arrow table using MinioHandler helper."""
    buffer = io.BytesIO()
    pq.write_table(table, buffer, compression=compression)
    buffer.seek(0)
    # Use upload_table if available to preserve schema
    if hasattr(minio_client, "upload_table"):
        return minio_client.upload_table(table=table, object_path=object_path, format="parquet", compression=compression)

    # Fallback to put_object
...
if __name__ == "__main__":
    ...
    try:
        # Minimal example configuration - override via env vars if needed
        mongo_cfg = {
            'host': os.getenv('MONGO_HOST', 'localhost'),
            # 'port': int(os.getenv('MONGO_PORT', '27017')),
            # 'username': os.getenv('MONGO_USER', "root"),
            # 'password': os.getenv('MONGO_PASSWORD', "pass123"),
        }

        # pymongo accepts host/port or a connection string; adapt if needed
