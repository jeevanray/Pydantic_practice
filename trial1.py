...
# -----------------------------------------------------------------------------
# FIXED: DB Helpers with mandatory error raising
# -----------------------------------------------------------------------------
def connect_to_oracle(oracle_conf: Dict[str, Any]) -> oracledb.Connection:
    """
    STRICT: Connect to Oracle database with retry logic and MANDATORY error raising.
    """
...
# -----------------------------------------------------------------------------
# STRICT: Audit helpers with mandatory blocking updates
# -----------------------------------------------------------------------------
...
def get_historic_load_status(config_audit: Dict[str, Any], source_table: str,
    ...
    """
    STRICT: Get historic load status - returns SINGLE next delta to process.
    Must complete current delta before advancing to next.
    """
    ...
    try:
        ...
        logger.info("STRICT: Processing delta %s (remaining: %s)", next_delta_str, len(unprocessed_deltas))
        ...
        with audit_conn.cursor() as cur:
            ...
            if row:
                ...
                # STRICT: Only return non-completed jobs
                if status == 'COMPLETED':
                    logger.info("Delta %s already COMPLETED - this should not happen in strict mode", next_delta_str)
                    return []
...
# -----------------------------------------------------------------------------
# STRICT: Core Extraction Function with mandatory completion
# -----------------------------------------------------------------------------
def oracle_to_minio_parquet(
    ...
    *,
    ...
    """
    STRICT: Enhanced extraction with mandatory completion and error raising.
    """
...
# -----------------------------------------------------------------------------
# STRICT: Historic Processing with while True loop until error
# -----------------------------------------------------------------------------
def process_historic_strict_single_delta(
    oracle_config: Dict[str, Any],
    minio_config: Dict[str, Any],
    config_audit: Dict[str, Any],
    table_name: str,
    ...
    """
    STRICT: Process single historic delta with while True loop until completion or error.
    This function processes ONLY ONE delta value completely or raises error.
    """
    table_name_uc = table_name.upper()
    logger.info("STRICT: Starting historic processing for %s", table_name_uc)
    
    # Get the current delta to process
    dates = get_load_status_and_dates(
        config_audit, table_name_uc, current_business_loaddt,
        'historic', delta_column, oracle_config
    )
    
    if not dates:
        logger.info("STRICT: No historic deltas to process for %s", table_name_uc)
        return
    
    date_info = dates[0]
    delta_value = date_info.get("delta_column_value")
    initial_status = date_info.get("status")
    
    logger.info("STRICT: Processing delta %s (initial status: %s)", delta_value, initial_status)
    
    # STRICT: while True loop until completion or error
    iteration = 0
    ...
    while True:
        ...
        if iteration > max_iterations:
            error_msg = f"CRITICAL: Historic processing exceeded {max_iterations} iterations for delta {delta_value}"
            logger.error(error_msg)
            raise RuntimeError(error_msg)
        
        logger.info("STRICT: Processing iteration %s for delta %s", iteration, delta_value)
        
        # Get current status
        current_dates = get_load_status_and_dates(
            config_audit, table_name_uc, current_business_loaddt,
            'historic', delta_column, oracle_config
        )
        
        if not current_dates:
            logger.info("STRICT: Delta %s completed - no more work to do", delta_value)
            break
        
        current_info = current_dates[0]
        current_status = current_info.get("status")
        current_restart_point = int(current_info.get("restart_point") or 0)
        
        logger.info("STRICT: Delta %s - status=%s, restart_point=%s", 
                   delta_value, current_status, current_restart_point)
        
        if current_status == "COMPLETED":
            logger.info("STRICT: Delta %s completed successfully", delta_value)
            break
        ...
        try:
            oracle_to_minio_parquet(
                ...
                business_loaddt=current_info["business_loaddt"],
                chunk_size=chunk_size,
                compression=compression,
                order_by=order_by,
                restart_point=current_restart_point,
                load_type='historic',
            ...
            logger.info("STRICT: Extraction completed for delta %s, checking final status...", delta_value)
            
            # Brief pause to ensure DB consistency
            time.sleep(1)
            
        except Exception as e:
            error_msg = f"CRITICAL: Failed to process historic delta {delta_value}: {e}"
            logger.error(error_msg)
            raise RuntimeError(error_msg)
    
    logger.info("STRICT: Historic delta %s processing completed successfully", delta_value)

# -----------------------------------------------------------------------------
# STRICT: Enhanced Orchestrator with mandatory completion
# -----------------------------------------------------------------------------
def process_oracle_to_minio_with_dependencies(
    ...
    *,
    ...
    """
    STRICT: Enhanced driver with mandatory completion per delta.
    """
    table_name_uc = table_name.upper()
    ...
    if load_type == 'historic':
        # STRICT: Process single delta with while True loop
        if not delta_column:
            raise ValueError("delta_column is required for historic load type")
        
        process_historic_strict_single_delta(
            oracle_config=oracle_config,
            minio_config=minio_config,
            config_audit=config_audit,
            table_name=table_name_uc,
        ...
        # Original delta processing logic
        dates = get_load_status_and_dates(
            config_audit, table_name_uc, current_business_loaddt, 
        ...
        for date_info in dates:
            ...
            logger.info("STRICT: Processing %s status=%s restart_point=%s delta_value=%s load_type=%s",
